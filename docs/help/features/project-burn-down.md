...
Shows how much work remains and when the project is likely to finish based on your recent pace.
...
When a project has a **target date**, a straight line from the starting total down to zero at the target date. This is the pace you'd need to maintain to finish on time.

### Projected Line (dashed)

A forward-looking trend line that extrapolates from your recent velocity to estimate **when remaining work will reach zero**.

- **Green** if projected to finish before the target date (or no target date set)
- **Amber** if projected to finish after the target date
