...
### Example: Create a recurring template
...
curl -X POST \
  ...
  -d '{
    "title": "Weekly review",
    "cronExpression": "weekly:1",
    "taskDefaults": {"contextId": "...", "estimatedMins": 30}
  }' \
  https://your-tandem.example.com/api/recurring-templates
```

Schedule formats: `daily`, `weekdays`, `weekly:1` (0=Sun..6=Sat), `biweekly:1`, `monthly:15`, `quarterly:1`, `yearly:3:15` (month:day), `every_n_days:3`.

### Example: Update a task by ID
...
curl -X PATCH \
  ...
  https://your-tandem.example.com/api/tasks/TASK_ID
```
...
The full OpenAPI 3.1 spec is available at [`/openapi.json`](/openapi.json) — requires a Bearer token (or session cookie). AI agents can fetch this to discover all available endpoints, request/response schemas, and authentication requirements.

**For AI agents getting started**, fetch the spec and the help docs:

```bash
# 1. Get the OpenAPI spec (requires Bearer token)
curl -H "Authorization: Bearer tnm_..." \
  https://your-tandem.example.com/openapi.json

# 2. List all help articles
curl -H "Authorization: Bearer tnm_..." \
  https://your-tandem.example.com/api/help

# 3. Read a specific help article
curl -H "Authorization: Bearer tnm_..." \
  https://your-tandem.example.com/api/help/public-rest-api
```

Help articles are returned in Markdown format and cover all features, GTD methodology, and admin guides.
...
| `/api/tasks` | PATCH | Update a task (id in body, supports optimistic concurrency) |
| `/api/tasks/{id}` | PATCH | Update a task (id in URL) |
...
| `/api/recurring-templates` | GET | List your recurring templates |
| `/api/recurring-templates` | POST | Create a recurring template (use `cronExpression` for schedule) |
| `/api/recurring-templates/{id}` | GET | Get a recurring template |
| `/api/recurring-templates/{id}` | PATCH | Update a recurring template |
| `/api/recurring-templates/{id}` | DELETE | Delete a recurring template |
...
| `/api/help` | GET | List help articles (filter by category, tag, search) |
| `/api/help/{slug}` | GET | Get a help article (Markdown content) |
