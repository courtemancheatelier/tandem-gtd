...
category: Features
tags: [projects, templates, automation, reuse, teams]
sortOrder: 9
---

# Project Templates

Project templates let you start new projects from proven structures instead of building from scratch. Tandem has three levels of templates:

- **System Templates** — built-in templates available to all users
- **Team Templates** — templates shared with your team, built from real projects you've run
- **Personal Templates** — your own private templates
...
System templates cannot be deleted or modified by regular users. Admins can manage system template visibility from the admin settings.
...
Saved templates appear under **My Templates** in the template library. You can delete personal templates by hovering and clicking the trash icon.

### Saving as a Team Template

If you're a member of one or more teams, you'll see a **Save to** dropdown when saving a template:

- **Personal (just me)** — only you can see and use it
- **Team Name** — any member of that team can use it; team admins can delete it

Team templates appear under the **Team Templates** section in the template library, grouped by team name. This is the recommended way to build reusable workflows — save a project that worked well, and the whole team benefits.
...
Save as a team template:

```bash
curl -X POST \
  -H "Authorization: Bearer tnm_abc123..." \
  -H "Content-Type: application/json" \
  -d '{"title": "Sprint Template", "description": "Standard sprint", "teamId": "TEAM_ID"}' \
  https://your-tandem.example.com/api/projects/PROJECT_ID/save-as-template
```
...
If your template library is cluttered with system or team templates you don't use, you can hide them:

1. In the template library, hover over a system or team template
2. Click the **eye-off** icon to hide it
3. The template disappears from your view (other users are unaffected)

To see and restore hidden templates, click **Show hidden** at the top of the library. Hidden templates appear greyed out with an **eye** icon to unhide them.

Personal templates can't be hidden — just delete them if you don't need them.
...
curl -X POST \
  -H "Authorization: Bearer tnm_abc123..." \
  https://your-tandem.example.com/api/project-templates/TEMPLATE_ID/hide
```
