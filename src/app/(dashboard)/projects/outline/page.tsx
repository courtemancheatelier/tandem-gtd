...
export default function ProjectOutlinePage() {
  ...
  const savedOutlineFilters = loadFilters("outline");
  const [includeSomeday, setIncludeSomeday] = useState(savedOutlineFilters?.someday === "true");
  const [areaFilter, setAreaFilter] = useState<string>(savedOutlineFilters?.area ?? "__all__");
  const [teamFilter, setTeamFilter] = useState<string>(savedOutlineFilters?.team ?? "__all__");
  const outlineRef = useRef<OutlineActions | null>(null);
  const [, forceRender] = useState(0);
  const [projectFilter, setProjectFilter] = useState<string>(savedOutlineFilters?.project ?? "__all__");
  ...
  useEffect(() => {
    saveFilters("outline", {
      someday: includeSomeday ? "true" : null,
      area: areaFilter !== "__all__" ? areaFilter : null,
      team: teamFilter !== "__all__" ? teamFilter : null,
      project: projectFilter !== "__all__" ? projectFilter : null,
    });
  }, [includeSomeday, areaFilter, teamFilter, projectFilter]);
