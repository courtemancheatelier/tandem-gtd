...
export async function GET(
  ...
  const team = await prisma.team.findUnique({
    ...
    include: {
      ...
      parentTeam: {
        select: { id: true, name: true, icon: true, _count: { select: { members: true } } },
      },
      childTeams: {
        select: {
          id: true,
...
export async function DELETE(
  ...
  try {
    ...
    if (error instanceof Error) {
      if (error.message === "Team not found") return notFound("Team not found");
      if (error.message.includes("admin")) return NextResponse.json({ error: error.message }, { status: 403 });
      if (error.message.includes("child groups")) return NextResponse.json({ error: error.message }, { status: 400 });
    }
    throw error;
  }
}
