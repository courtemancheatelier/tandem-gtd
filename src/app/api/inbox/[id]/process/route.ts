...
export async function POST(
  ...
  if (data.decision === "not_actionable") {
    ...
    // Carry external link from inbox item to task (e.g. from share target)
    if (inboxItem.externalLinkUrl) {
      await prisma.task.update({
        where: { id: task.id },
        data: {
          externalLinkUrl: inboxItem.externalLinkUrl,
          externalLinkLabel: inboxItem.externalLinkLabel,
        },
      });
    }
