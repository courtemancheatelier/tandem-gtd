...
export async function GET(
  ...
  const project = await prisma.project.findFirst({
    ...
    include: {
      ...
      childProjects: {
        select: {
          id: true, title: true, status: true, type: true, rollupProgress: true, sortOrder: true, version: true,
          _count: { select: { tasks: true, childProjects: true } },
        },
        orderBy: { sortOrder: "asc" },
      },
      rsvpEvent: { select: { id: true } },
    },
  });
...
export async function PATCH(
  ...
  const { note: rawNote, ...projectData } = body;
  ...
  const trimmedNote = typeof rawNote === "string" && rawNote.trim() ? rawNote.trim() : undefined;
