...
export async function GET(
  ...
  // Adjust completion velocity by multiplier if available
  const adjustedCompletionVelocity = velocityMultiplier
    ? completionVelocityPerWeek / velocityMultiplier
    : completionVelocityPerWeek;

  const gap = scope - completed;
  const netVelocity = adjustedCompletionVelocity - scopeVelocityPerWeek;
  ...
  if (adjustedCompletionVelocity > 0) {
    ...
    for (let w = 1; w <= 12; w++) {
      ...
      const val = startProjected + adjustedCompletionVelocity * w;
  ...
  if (gap > 0) {
    if (isConverging) {
      ...
      convergence = {
        ...
        completionVelocityPerWeek: round2(adjustedCompletionVelocity),
