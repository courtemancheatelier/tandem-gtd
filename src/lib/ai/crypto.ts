...
function getKey(): Buffer {
  const dedicatedKey = process.env.TANDEM_ENCRYPTION_KEY;
  const secret = dedicatedKey || process.env.NEXTAUTH_SECRET;
  if (!secret) {
    throw new Error("TANDEM_ENCRYPTION_KEY or NEXTAUTH_SECRET must be set");
  }
  if (!dedicatedKey && !encryptionKeyWarned) {
    encryptionKeyWarned = true;
    console.warn(
      "[security] TANDEM_ENCRYPTION_KEY is not set — falling back to NEXTAUTH_SECRET for encryption. " +
      "Set TANDEM_ENCRYPTION_KEY in production to isolate encryption from session signing."
    );
  }
