...
async function loadTemplateMeta(
  ...
  try {
    ...
    // Sanitize against path traversal (Finding 1.3)
    const baseDir = resolve(process.cwd(), "docs/templates");
    const filePath = resolve(baseDir, normalize(sourceFile));
...
export async function saveProjectAsTemplate(
  ...
  // Validate team ADMIN role if teamId provided (Finding 1.2)
  if (options.teamId) {
    const membership = await prisma.teamMember.findFirst({
      where: { teamId: options.teamId, userId },
    });
    if (!membership) throw new Error("You are not a member of this team");
    if (membership.role !== "ADMIN") throw new Error("Admin role required to create team templates");
  }
