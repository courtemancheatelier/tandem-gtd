 ...
 * Blocks deletion if team has child teams.
 */
export async function deleteTeam(
  ...
  const existing = await prisma.team.findUnique({
    where: { id: teamId },
    include: { _count: { select: { childTeams: true } } },
  });
  if (!existing) throw new Error("Team not found");

  if (existing._count.childTeams > 0) {
    throw new Error("Cannot delete a team that has child groups. Remove or reassign child groups first.");
  }
...
/**
 * List all teams a user belongs to, with member count, role, and hierarchy data.
 */
export async function getUserTeams(userId: string) {
  ...
  return memberships.map((m) => ({
    ...
    parentTeam: m.team.parentTeam,
    childTeams: m.team.childTeams.map((c) => ({
      id: c.id,
      name: c.name,
      icon: c.icon,
