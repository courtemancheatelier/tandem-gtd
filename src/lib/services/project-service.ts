...
export async function updateProject(
  ...
  if (updates.status && updates.status !== existing.status) {
    // Set startDate when project becomes ACTIVE (if blank)
    if (updates.status === "ACTIVE" && !existing.startDate && !updates.startDate) {
      updates.startDate = new Date().toISOString();
    }
    // Set endDate when project is completed or dropped
    if ((updates.status === "COMPLETED" || updates.status === "DROPPED") && !updates.endDate) {
      updates.endDate = new Date().toISOString();
    }
    // Clear endDate when reactivating from completed/dropped
    if (
