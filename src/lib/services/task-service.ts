...
import { completeDelegation } from "@/lib/services/delegation-service";
...
import { isForwardShift, computeDriftDays } from "@/lib/drift/utils";
...
export interface ActorContext {
  ...
  actorId?: string | null;
  source?: "MANUAL" | "MCP" | "AI_EMBED" | "CASCADE" | "SCHEDULER" | "API" | "IMPORT" | "TEAM_SYNC";
  triggeredBy?: string;
  message?: string;
}
...
export async function updateTask(
  ...
  const task = await prisma.$transaction(async (tx: TxClient) => {
    ...
    if (Object.keys(changes).length > 0) {
      ...
      // ── Commitment Drift counters ──────────────────────────────────────
      const driftUpdates: Record<string, unknown> = {};

      // Deferral: scheduledDate was set (DEFERRED event type)
      if (eventType === "DEFERRED") {
        driftUpdates.deferralCount = { increment: 1 };
      }

      // Due date push: due date moved forward
      if (changes.dueDate && isForwardShift(changes.dueDate)) {
        driftUpdates.dueDatePushCount = { increment: 1 };
        // Set originalDueDate only on the first push
        if (!existing.originalDueDate && changes.dueDate.old) {
          driftUpdates.originalDueDate = new Date(changes.dueDate.old as string);
        }
        // Recompute totalDriftDays
        const origDate = existing.originalDueDate ?? (changes.dueDate.old ? new Date(changes.dueDate.old as string) : null);
        if (origDate) {
          driftUpdates.totalDriftDays = computeDriftDays(origDate, changes.dueDate.new as string);
        }
      }
...
export async function completeTask(
  ...
  // Auto-complete delegation if this task was delegated
  await completeDelegation(taskId);

  // Finalize drift: compute totalDriftDays using originalDueDate vs completedAt
  if (task.originalDueDate && task.completedAt) {
    const finalDrift = computeDriftDays(task.originalDueDate, task.completedAt);
    if (finalDrift !== task.totalDriftDays) {
      await prisma.task.update({
        where: { id: taskId },
        data: { totalDriftDays: finalDrift },
      });
    }
  }
  ...
  // Recycle recurring task if this task was generated from a routine
  if (task.routineId) {
    // Check if this is a simple routine (no windows) — those use the recycle flow
    const routineForRecycle = await prisma.routine.findUnique({
      where: { id: task.routineId },
      select: { id: true, _count: { select: { windows: true } } },
    });

    if (routineForRecycle && routineForRecycle._count.windows === 0) {
