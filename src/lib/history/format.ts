...
const EVENT_DESCRIPTIONS: Record<string, string> = {
  ...
  // Thread events
  THREAD_OPENED: "Opened thread",
  THREAD_RESOLVED: "Resolved thread",
  // Decision events
  DECISION_REQUESTED: "Requested decision",
...
export function formatEventDescription(event: EventInput): string {
  ...
  if (event.eventType === "TIMER_SESSION") {
    const duration =
      typeof changes.durationMin?.new === "number"
        ? changes.durationMin.new
        : 0;
...
function formatSourceSuffix(source: string, actorType: string): string {
  ...
  if (source === "TEAM_SYNC") return "(team note)";
