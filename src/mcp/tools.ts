 ...
 *   tandem_routine_list                - List routines (simple cards + windowed)
 *   tandem_routine_create              - Create a routine (simple or windowed)
 *   tandem_routine_update              - Update a routine
 *   tandem_routine_delete              - Delete a routine
 *   tandem_card_file_list              - Get card file view (overdue/today/upcoming)
 *   (tandem_recurring_template_* aliases kept for backward compatibility)
 */
...
const TOOLS = [
    ...
    inputSchema: {
      ...
      properties: {
        ...
        note: {
          type: "string",
          description:
            "Optional context note for the team explaining why this task was completed or any relevant details. Saved on the task event for team visibility.",
        },
        actualMinutes: {
          type: "number",
          description:
            "Optional actual time spent on the task in minutes. Recorded alongside completion for duration tracking.",
        },
      },
      required: ["taskId"],
    },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        startDate: {
          type: "string",
          description: "Project start date in ISO 8601 format (e.g. 2026-03-22T00:00:00Z)",
        },
        endDate: {
          type: "string",
          description: "Project end date in ISO 8601 format (e.g. 2026-04-11T00:00:00Z)",
        },
        tasks: {
          type: "array",
          description:
    ...
    inputSchema: {
      ...
      properties: {
        ...
        actualMinutes: {
          type: "number",
          description:
            "Actual time spent on the task in minutes. Set to 0 to clear.",
        },
        externalLinkUrl: {
          type: "string",
          description: "Full URL for external reference (include https://). Set to empty string to clear.",
        },
        externalLinkLabel: {
          type: "string",
          description: "Display label for the link, e.g. 'Project Brief'. Optional — defaults to hostname.",
        },
      },
      required: ["taskId"],
    },
  ...
  // ── Decision Tools ───────────────────────────────────────────────────
  {
    ...
    description:
      "Create a decision request or quick poll on a team task or project. Creates a QUESTION thread, adds respondents, and creates WaitingFor for the requester. Use decisionType POLL with options for multi-option polls.",
    inputSchema: {
      type: "object" as const,
      properties: {
        ...
        taskId: { type: "string", description: "Task to attach decision to" },
        projectId: { type: "string", description: "Project to attach decision to" },
        respondentIds: {
          type: "array",
          items: { type: "string" },
          description: "User IDs of people who should respond",
        },
        deadline: { type: "string", description: "ISO 8601 deadline for responses" },
        decisionType: {
          type: "string",
          enum: ["APPROVAL", "POLL"],
          description: "APPROVAL (default) for approve/reject voting, POLL for named options",
        },
        options: {
          type: "array",
          items: {
          ...
          description: "Poll options (required for POLL type, min 2)",
        },
        wikiSlug: { type: "string", description: "Wiki article slug to auto-update when decision resolves" },
      },
      required: ["question", "respondentIds"],
    },
  },
  {
    name: "tandem_decision_respond",
    description:
      "Submit a vote on an open decision. Votes: APPROVE, REJECT, COMMENT, DEFER. Can change your vote by calling again.",
    inputSchema: {
      type: "object" as const,
      properties: {
        ...
        vote: {
          type: "string",
          enum: ["APPROVE", "REJECT", "COMMENT", "DEFER"],
          description: "Your vote",
        },
        comment: { type: "string", description: "Optional comment with your vote" },
      },
  ...
  },
  {
    ...
    description:
      "List decisions awaiting your response. Returns OPEN decisions where you are a designated respondent and haven't voted yet.",
    inputSchema: {
      type: "object" as const,
      properties: {},
    },
  ...
  {
    name: "tandem_decision_resolve",
    description:
      "Resolve a decision with a summary. Only the requester can resolve. Resolves the parent thread and linked WaitingFor. For POLL decisions, optionally specify chosenOptionId.",
    inputSchema: {
      type: "object" as const,
      properties: {
        decisionId: { type: "string", description: "The decision ID to resolve" },
        resolution: { type: "string", description: "Resolution summary" },
        chosenOptionId: { type: "string", description: "For POLL decisions, the ID of the chosen option" },
      },
      required: ["decisionId", "resolution"],
    },
  },
  {
    name: "tandem_decision_vote_option",
    description:
      "Vote for an option on a POLL decision. One vote per person per decision — voting again changes your vote.",
    inputSchema: {
      type: "object" as const,
      properties: {
        decisionId: { type: "string", description: "The poll decision ID" },
        optionId: { type: "string", description: "The option ID to vote for" },
        comment: { type: "string", description: "Optional comment with your vote" },
      },
  ...
  {
    name: "tandem_decision_list",
    description:
      "List decisions for a project or team. Returns all decisions (open, resolved, withdrawn) ordered by status then date.",
    inputSchema: {
      type: "object" as const,
      properties: {
        projectId: { type: "string", description: "List decisions for this project" },
        teamId: { type: "string", description: "List decisions across all team projects" },
      },
    },
  },
  // ── Thread Tools ─────────────────────────────────────────────────────
  {
    ...
    description:
      "Create a new thread on a team task or project. Threads enable team conversations anchored to work items. BLOCKER threads auto-create WaitingFor entries.",
    inputSchema: {
      type: "object" as const,
      properties: {
        purpose: {
          ...
          description: "Thread purpose: QUESTION, BLOCKER (creates WaitingFor), UPDATE, or FYI (no inbox items for mentions)",
        },
        title: { type: "string", description: "Brief thread title" },
        message: { type: "string", description: "First message content" },
        taskId: { type: "string", description: "Task to attach thread to (mutually exclusive with projectId)" },
        projectId: { type: "string", description: "Project to attach thread to (mutually exclusive with taskId)" },
        mentions: {
          type: "array",
          items: { type: "string" },
          description: "User IDs to @-mention (creates inbox items unless FYI)",
        },
        setTaskWaiting: {
          type: "boolean",
          description: "For BLOCKER threads on tasks: also set the task status to WAITING",
        },
      },
      required: ["purpose", "title", "message"],
    },
  },
  {
    name: "tandem_thread_list",
    description:
      "List threads on a task or project. Returns open threads by default.",
    inputSchema: {
      type: "object" as const,
      properties: {
        taskId: { type: "string", description: "Filter by task ID" },
        projectId: { type: "string", description: "Filter by project ID" },
        includeResolved: { type: "boolean", description: "Include resolved threads (default: false)" },
      },
    },
  },
  {
    name: "tandem_thread_resolve",
    description:
      "Resolve a thread. For BLOCKER threads: auto-resolves linked WaitingFor and restores WAITING tasks to IN_PROGRESS.",
    inputSchema: {
      type: "object" as const,
      properties: {
        threadId: { type: "string", description: "The thread ID to resolve" },
      },
      required: ["threadId"],
    },
  },
  {
    name: "tandem_thread_reply",
    description:
      "Add a reply message to a thread. Reopens the thread if it was resolved.",
    inputSchema: {
      type: "object" as const,
      properties: {
  ...
  // =========================================================================
  {
    name: "tandem_routine_list",
    description:
      "List routines (simple recurring cards and windowed checklists). Shows schedule, progression settings, time-of-day config, skip streaks, and windows.",
    inputSchema: {
      type: "object" as const,
      properties: {
        activeOnly: {
          type: "boolean",
          description: "If true, only return active routines (default: false, returns all)",
        },
        type: {
          type: "string",
          enum: ["simple", "windowed", "all"],
          description: 'Filter by type: "simple" (no windows), "windowed" (has windows), "all" (default)',
        },
      },
    },
  },
  {
    name: "tandem_routine_create",
    description:
      "Create a routine. Simple routines (no windows) generate recurring task cards with optional progressive difficulty. Windowed routines have time-based checklist windows with items.",
    inputSchema: {
      type: "object" as const,
      properties: {
        title: { type: "string", description: "Routine title (used as task title for simple routines)" },
        description: { type: "string", description: "Optional description/notes" },
        cronExpression: {
          type: "string",
          description:
            'Schedule expression: "daily", "weekdays", "weekly:0"-"weekly:6" (0=Sun), "biweekly:0"-"biweekly:6", "monthly:1"-"monthly:31", "quarterly:1"-"quarterly:31", "yearly:1-12:1-31", "every_n_days:1"-"every_n_days:999"',
        },
        color: { type: "string", description: "Hex color for card styling (e.g., #6366f1)" },
        estimatedMins: { type: "number", description: "Time estimate in minutes" },
        targetTime: {
          type: "string",
          description: 'HH:MM — ideal time to do this task (for card ordering). E.g., "09:00"',
        },
        dueByTime: {
          type: "string",
          description: 'HH:MM — card shows as overdue after this time. E.g., "11:00". Must be after targetTime.',
        },
        progression: {
          type: "object",
          description: "Progressive difficulty config (auto-increasing targets over time). Simple routines only.",
          properties: {
            baseValue: { type: "number", description: "Starting value (e.g., 5)" },
            increment: { type: "number", description: "Amount to increase per period (e.g., 1)" },
            unit: { type: "string", description: 'Label for the value: "min", "laps", "reps", etc.' },
            frequency: {
              type: "string",
              enum: ["daily", "weekly", "monthly"],
              description: "How often to increase",
            },
          },
          required: ["baseValue", "increment", "unit", "frequency"],
        },
        areaId: { type: "string", description: "Area of responsibility ID" },
        goalId: { type: "string", description: "Goal ID" },
        taskDefaults: {
          type: "object",
          description: "Default values applied to generated tasks (simple routines only)",
          properties: {
      ...
      required: ["title", "cronExpression"],
    },
  },
  {
    name: "tandem_routine_update",
    description:
      "Update a routine. Supports changing schedule, progression, time-of-day, and all other settings. Pass null to clear optional fields.",
    inputSchema: {
      type: "object" as const,
      properties: {
        routineId: { type: "string", description: "The routine ID to update" },
        title: { type: "string" },
        description: { type: "string" },
        cronExpression: { type: "string", description: "New schedule expression" },
        isActive: { type: "boolean", description: "Activate or pause the routine" },
        color: { type: "string" },
        estimatedMins: { type: "number" },
        targetTime: { type: "string", description: "HH:MM or null to clear" },
        dueByTime: { type: "string", description: "HH:MM or null to clear" },
        progression: {
          type: "object",
          description: "Progressive difficulty config, or null to disable",
          properties: {
  ...
  {
    name: "tandem_routine_delete",
    description: "Delete a routine. Tasks already generated will remain but lose their routine link.",
    inputSchema: {
      type: "object" as const,
      properties: {
        routineId: { type: "string", description: "The routine ID to delete" },
      },
      required: ["routineId"],
    },
  },
  {
    name: "tandem_card_file_list",
    description:
      "Get the card file view showing recurring tasks split into overdue, today, and upcoming sections. Today's cards are sorted by targetTime. Includes progression targets and overdue-within-day status.",
    inputSchema: {
      type: "object" as const,
      properties: {},
    },
...
async function handleTaskComplete(args: {
  ...
  const actualMins = typeof args.actualMinutes === "number" && args.actualMinutes > 0
    ? Math.round(args.actualMinutes)
    : undefined;
  const completedTask = await prisma.task.update({
    ...
    data: {
      ...
      version: { increment: 1 },
      ...(actualMins !== undefined && { actualMinutes: actualMins }),
    },
  });
  ...
  const completionEvent = await prisma.taskEvent.create({
    data: {
      ...
      source: trimmedNote ? "TEAM_SYNC" : "MCP",
      message: trimmedNote,
    },
  });
  ...
  // Recycle recurring task if linked to a routine
  ...
  if (existing.routineId) {
    ...
    const recycled = await recycleRecurringTask(existing.routineId, existing.scheduledDate);
    if (recycled) {
      recycledTasks.push({
        id: recycled.id,
  ...
  return JSON.stringify(
    {
      tasks: tasks.map((t) => ({
        ...
        externalLinkUrl: t.externalLinkUrl,
...
async function handleTaskUpdate(args: {
  ...
  if (args.actualMinutes !== undefined) {
    data.actualMinutes = args.actualMinutes || null;
    changes.actualMinutes = { old: existing.actualMinutes, new: data.actualMinutes };
  }
  if (args.externalLinkUrl !== undefined) {
    data.externalLinkUrl = args.externalLinkUrl || null;
    changes.externalLinkUrl = { old: existing.externalLinkUrl, new: data.externalLinkUrl };
  }
  if (args.externalLinkLabel !== undefined) {
    data.externalLinkLabel = args.externalLinkLabel || null;
    changes.externalLinkLabel = { old: existing.externalLinkLabel, new: data.externalLinkLabel };
  }
  ...
  return JSON.stringify(
    {
      ...
      task: {
        ...
        notes: updated.notes,
        externalLinkUrl: updated.externalLinkUrl,
        externalLinkLabel: updated.externalLinkLabel,
...
async function handleProjectCreateFromTemplate(args: {
  ...
  if (!args.templateId) {
    ...
    const templates = await prisma.projectTemplate.findMany({
      where: {
        OR: [
          { isSystem: true, isGloballyHidden: false },
          { userId },
          ...(teamIds.length > 0 ? [{ teamId: { in: teamIds } }] : []),
        ],
        NOT: {
...
async function handleDecisionCreate(args: {
  question: string;
  context?: string;
  taskId?: string;
  projectId?: string;
  ...
  if (!args.taskId && !args.projectId) {
    return JSON.stringify({ error: "Either taskId or projectId is required" });
  }
  if (args.taskId && args.projectId) {
    return JSON.stringify({ error: "Provide taskId or projectId, not both" });
  }

  // Resolve the team context from the anchor
  let teamId: string | null = null;
  let anchorProjectId: string | null = args.projectId ?? null;
  if (args.taskId) {
    ...
    anchorProjectId = task.projectId;
  ...
  if (!teamId) return JSON.stringify({ error: "Decisions require a team context" });

  // Verify requester is a team member
  if (!(await mcpIsTeamMember(userId, teamId))) {
    return JSON.stringify({ error: "Not a member of this team" });
  }
  ...
  if (anchorProjectId) {
    const proj = await prisma.project.findUnique({
      ...
      select: { decisionsEnabled: true },
    });
    if (proj && !proj.decisionsEnabled) {
      return JSON.stringify({ error: "Decisions are disabled for this project" });
    }
  }

  const decisionType = (args.decisionType as "APPROVAL" | "POLL" | "QUICK_POLL") || "APPROVAL";
  const isPollType = decisionType === "POLL" || decisionType === "QUICK_POLL";
  if (isPollType && (!args.options || args.options.length < 2)) {
    return JSON.stringify({ error: "Poll decisions require at least 2 options" });
  }

  const typeLabel = isPollType ? "Poll" : "Decision";

  const result = await prisma.$transaction(async (tx) => {
    const thread = await tx.thread.create({
      data: {
        purpose: "QUESTION",
        title: args.question.slice(0, 100),
        createdById: userId,
        taskId: args.taskId,
    ...
    // Create options for POLL type
    if (decisionType === "POLL" && args.options) {
      for (let i = 0; i < args.options.length; i++) {
        await tx.decisionOption.create({
          data: {
            decisionId: decision.id,
            label: args.options[i].label,
            description: args.options[i].description,
            sortOrder: i,
          },
        });
      }
    ...
    // Only add respondents who are verified team members
    const validRespondentIds: string[] = [];
    for (const rid of args.respondentIds) {
      if (await mcpIsTeamMember(rid, teamId!)) {
        validRespondentIds.push(rid);
        await tx.decisionRespondent.create({
          data: { decisionId: decision.id, userId: rid },
        });
      }
    }
    ...
    await tx.waitingFor.create({
      data: {
        userId,
        description: `${typeLabel}: ${args.question.slice(0, 100)}`,
        person: "Team",
        dueDate: args.deadline ? new Date(args.deadline) : undefined,
        threadId: thread.id,
      },
    });
  ...
  return JSON.stringify({ success: true, threadId: result.thread.id, decisionId: result.decision.id, decisionType });
}

async function handleDecisionRespond(args: {
  decisionId: string;
  vote: string;
  comment?: string;
}): Promise<string> {
  ...
  if (decision.status !== "OPEN") return JSON.stringify({ error: "Decision is not open" });
  if (!decision.respondents.some((r) => r.userId === userId)) {
    return JSON.stringify({ error: "Not a designated respondent" });
  }

  const response = await prisma.decisionResponse.upsert({
    where: { decisionId_userId: { decisionId: args.decisionId, userId } },
    create: {
      decisionId: args.decisionId,
      userId,
      vote: args.vote as "APPROVE" | "REJECT" | "COMMENT" | "DEFER",
...
async function handleDecisionListPending(): Promise<string> {
  const prisma = getPrisma();
  const userId = getUserId();
...
async function handleDecisionResolve(args: {
  decisionId: string;
  resolution: string;
  ...
  if (!decision) return JSON.stringify({ error: "Decision not found" });
  if (decision.requesterId !== userId) return JSON.stringify({ error: "Only the requester can resolve" });
  if (decision.status !== "OPEN") return JSON.stringify({ error: "Decision is not open" });

  await prisma.$transaction(async (tx) => {
    // Mark chosen option if provided
    if (args.chosenOptionId && decision.decisionType === "POLL") {
      const option = await tx.decisionOption.findFirst({
        where: { id: args.chosenOptionId, decisionId: args.decisionId },
      });
    ...
    await tx.decisionRequest.update({
      where: { id: args.decisionId },
      data: { status: "RESOLVED", resolution: args.resolution, resolvedAt: new Date() },
    });
  ...
  return JSON.stringify({ success: true, decisionId: args.decisionId, resolved: true });
}

async function handleDecisionVoteOption(args: {
  decisionId: string;
  optionId: string;
  comment?: string;
}): Promise<string> {
  ...
  if (!decision) return JSON.stringify({ error: "Decision not found" });
  if (decision.status !== "OPEN") return JSON.stringify({ error: "Decision is not open" });
  if (decision.decisionType !== "POLL") return JSON.stringify({ error: "This decision is not a poll" });
  if (!decision.respondents.some((r) => r.userId === userId)) {
    return JSON.stringify({ error: "Not a designated respondent" });
  }
  if (!decision.options.some((o) => o.id === args.optionId)) {
    return JSON.stringify({ error: "Option does not belong to this decision" });
  }

  const vote = await prisma.decisionOptionVote.upsert({
    where: { decisionId_voterId: { decisionId: args.decisionId, voterId: userId } },
    create: {
      decisionId: args.decisionId,
  ...
  return JSON.stringify({ success: true, vote });
}

async function handleDecisionList(args: {
  projectId?: string;
  teamId?: string;
}): Promise<string> {
  const prisma = getPrisma();
  ...
  if (!args.projectId && !args.teamId) {
    return JSON.stringify({ error: "Exactly one of projectId or teamId is required" });
  }
  if (args.projectId && args.teamId) {
    return JSON.stringify({ error: "Provide only one of projectId or teamId, not both" });
  }
  ...
  const include = {
    ...
    thread: { select: { id: true, taskId: true, projectId: true } },
  };
  ...
  if (args.projectId) {
    ...
    if (!project?.teamId) return JSON.stringify({ error: "Project is not part of a team" });
    const isMember = await prisma.teamMember.findFirst({
      where: { teamId: project.teamId, userId },
    });
    if (!isMember) return JSON.stringify({ error: "Not a member of this team" });

    decisions = await prisma.decisionRequest.findMany({
      where: {
        OR: [
          { thread: { projectId: args.projectId } },
          { thread: { task: { projectId: args.projectId } } },
        ],
      },
      include,
    ...
    const isMember = await prisma.teamMember.findFirst({
      where: { teamId: args.teamId!, userId },
    });
    if (!isMember) return JSON.stringify({ error: "Not a member of this team" });
  ...
  const lines = [
    `${decisions.length} decisions (${open} open, ${resolved} resolved, ${withdrawn} withdrawn)`,
    "",
  ];

  for (const d of decisions) {
    const respondentCount = d.respondents.length;
    const responseCount = d.responses.length;
    lines.push(
      `[${d.status}] "${d.question}" — by ${d.requester.name ?? "Unknown"} | ${responseCount}/${respondentCount} responses`
    );
  }
...
async function handleThreadCreate(args: {
  purpose: string;
  title: string;
  message: string;
  taskId?: string;
  ...
  const thread = await prisma.$transaction(async (tx) => {
    const thread = await tx.thread.create({
      data: {
        purpose: args.purpose as "QUESTION" | "BLOCKER" | "UPDATE" | "FYI",
        title: args.title,
        createdById: userId,
        taskId: args.taskId,
        projectId: args.projectId,
    ...
    if (args.purpose === "BLOCKER") {
      await tx.waitingFor.create({
        data: {
          userId,
          description: `Blocker thread: ${args.title}`,
          person: "Team",
          threadId: thread.id,
        },
      });

      if (args.setTaskWaiting && args.taskId) {
        await tx.task.update({
          where: { id: args.taskId },
  ...
  const full = await prisma.thread.findUnique({
    ...
    include: {
      ...
      messages: { include: { author: { select: { id: true, name: true } } } },
    },
  });

  return JSON.stringify({ success: true, thread: full });
}

async function handleThreadList(args: {
  taskId?: string;
  projectId?: string;
  includeResolved?: boolean;
}): Promise<string> {
  ...
  if (args.projectId) where.projectId = args.projectId;
  if (!args.includeResolved) where.isResolved = false;
...
async function handleThreadResolve(args: {
  threadId: string;
}): Promise<string> {
  const prisma = getPrisma();
  const userId = getUserId();
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (thread.waitingFors.length > 0) {
      await tx.waitingFor.updateMany({
        where: { threadId: args.threadId, isResolved: false },
        data: { isResolved: true, resolvedAt: new Date() },
      });
    }
  ...
  return JSON.stringify({ success: true, threadId: args.threadId, resolved: true });
}

async function handleThreadReply(args: {
  threadId: string;
  content: string;
  mentions?: string[];
}): Promise<string> {
  ...
  const message = await prisma.$transaction(async (tx) => {
    if (thread.isResolved) {
      await tx.thread.update({
        where: { id: args.threadId },
        data: { isResolved: false, resolvedAt: null, resolvedById: null },
      });
    }

    return tx.threadMessage.create({
      data: {
...
// =============================================================================
// Routine & Card File Handlers
// =============================================================================

async function handleRoutineList(args: {
  activeOnly?: boolean;
  type?: string;
}): Promise<string> {
  const prisma = getPrisma();
  ...
  if (args.type === "simple") where.windows = { none: {} };
  else if (args.type === "windowed") where.windows = { some: {} };
  ...
  const { scheduleLabel, getProgressionValue, formatProgression } = await import("@/lib/recurring");

  const mapped = routines.map((t) => {
    const isSimple = t.windows.length === 0;
    const base: Record<string, unknown> = {
      id: t.id,
      title: t.title,
      description: t.description,
      cronExpression: t.cronExpression,
      scheduleLabel: scheduleLabel(t.cronExpression),
      isActive: t.isActive,
      nextDue: t.nextDue?.toISOString() ?? null,
      lastGenerated: t.lastGenerated?.toISOString() ?? null,
      color: t.color,
      estimatedMins: t.estimatedMins,
      type: isSimple ? "simple" : "windowed",
      area: t.area,
      goal: t.goal,
    };
    if (isSimple) {
      base.targetTime = t.targetTime;
      base.dueByTime = t.dueByTime;
      base.skipStreak = t.skipStreak;
      if (t.progressionBaseValue != null && t.progressionIncrement != null && t.progressionUnit && t.progressionFrequency && t.progressionStartDate) {
        const currentValue = getProgressionValue({
          baseValue: t.progressionBaseValue,
          increment: t.progressionIncrement,
          unit: t.progressionUnit,
          frequency: t.progressionFrequency,
          startDate: t.progressionStartDate,
        });
        base.progression = {
          baseValue: t.progressionBaseValue,
          increment: t.progressionIncrement,
          unit: t.progressionUnit,
          frequency: t.progressionFrequency,
          startDate: t.progressionStartDate.toISOString(),
          currentValue,
          label: formatProgression(currentValue, t.progressionIncrement, t.progressionUnit, t.progressionFrequency),
        };
      }
    } else {
      base.windows = t.windows.map((w) => ({
        id: w.id,
        title: w.title,
  ...
  return JSON.stringify({ routines: mapped, count: mapped.length });
}

async function handleRoutineCreate(args: {
  title: string;
  description?: string;
  ...
  targetTime?: string;
  dueByTime?: string;
  progression?: { baseValue: number; increment: number; unit: string; frequency: string };
  areaId?: string;
  goalId?: string;
  taskDefaults?: { projectId?: string; contextId?: string; energyLevel?: string; estimatedMins?: number };
}): Promise<string> {
  const prisma = getPrisma();
  const userId = getUserId();
  const { getNextOccurrence, generateTaskFromTemplate } = await import("@/lib/recurring");

  const nextDue = getNextOccurrence(args.cronExpression, new Date());
  ...
  const routine = await prisma.routine.create({
    data: {
      ...
      estimatedMins: args.estimatedMins ?? null,
      targetTime: args.targetTime || null,
      dueByTime: args.dueByTime || null,
      ...(args.progression ? {
        progressionBaseValue: args.progression.baseValue,
        progressionIncrement: args.progression.increment,
        progressionUnit: args.progression.unit,
        progressionFrequency: args.progression.frequency,
        progressionStartDate: new Date(),
      } : {}),
      areaId: args.areaId || null,
      goalId: args.goalId || null,
      taskDefaults: args.taskDefaults ?? undefined,
      userId,
    },
  });
  ...
  await generateTaskFromTemplate({
    ...
    description: args.description || null,
    cronExpression: args.cronExpression,
    taskDefaults: (args.taskDefaults as Record<string, unknown> | null) ?? null,
    nextDue: now,
    isActive: true,
    lastGenerated: null,
  });
...
async function handleRoutineUpdate(args: {
  routineId: string;
  templateId?: string; // backward compat alias
  title?: string;
  description?: string;
  cronExpression?: string;
  isActive?: boolean;
  ...
  progression?: { baseValue: number; increment: number; unit: string; frequency: string } | null;
  areaId?: string | null;
  goalId?: string | null;
  taskDefaults?: { projectId?: string; contextId?: string; energyLevel?: string; estimatedMins?: number } | null;
}): Promise<string> {
  const prisma = getPrisma();
  const userId = getUserId();
  const id = args.routineId || args.templateId;
  if (!id) return JSON.stringify({ error: "routineId is required" });

  const existing = await prisma.routine.findFirst({
    where: { id, userId },
  });
  if (!existing) return JSON.stringify({ error: "Routine not found" });
  ...
  if (args.estimatedMins !== undefined) data.estimatedMins = args.estimatedMins;
  if (args.targetTime !== undefined) data.targetTime = args.targetTime;
  if (args.dueByTime !== undefined) data.dueByTime = args.dueByTime;
  if (args.areaId !== undefined) data.areaId = args.areaId;
  if (args.goalId !== undefined) data.goalId = args.goalId;
  if (args.taskDefaults !== undefined) data.taskDefaults = args.taskDefaults;

  if (args.cronExpression !== undefined) {
    const { getNextOccurrence } = await import("@/lib/recurring");
    data.cronExpression = args.cronExpression;
    data.nextDue = getNextOccurrence(args.cronExpression, new Date());
  }

  if (args.progression !== undefined) {
    if (args.progression === null) {
      data.progressionBaseValue = null;
      data.progressionIncrement = null;
      data.progressionUnit = null;
      data.progressionFrequency = null;
      data.progressionStartDate = null;
    } else {
      data.progressionBaseValue = args.progression.baseValue;
      data.progressionIncrement = args.progression.increment;
      data.progressionUnit = args.progression.unit;
      data.progressionFrequency = args.progression.frequency;
      data.progressionStartDate = existing.progressionStartDate ?? new Date();
    }
  }
  ...
  return JSON.stringify({
    ...
    routine: {
      ...
      cronExpression: routine.cronExpression,
      nextDue: routine.nextDue?.toISOString() ?? null,
    },
  });
}

async function handleRoutineDelete(args: {
  routineId: string;
  templateId?: string; // backward compat alias
}): Promise<string> {
  const prisma = getPrisma();
  ...
  return JSON.stringify({ success: true, deletedId: id });
}

async function handleCardFileList(): Promise<string> {
  const prisma = getPrisma();
  const userId = getUserId();
  const { scheduleLabel, getProgressionValue, formatProgression } = await import("@/lib/recurring");
  ...
  const tasks = await prisma.task.findMany({
    ...
    include: {
      ...
      routine: {
        select: {
          id: true, cronExpression: true, color: true, estimatedMins: true, skipStreak: true,
          targetTime: true, dueByTime: true,
          progressionBaseValue: true, progressionIncrement: true, progressionUnit: true,
          progressionFrequency: true, progressionStartDate: true,
        },
      },
    },
  ...
  for (const task of tasks) {
    ...
    if (!scheduled || scheduled < startOfToday) overdue.push(task);
    else if (scheduled >= startOfToday && scheduled < endOfToday) today.push(task);
    else upcoming.push(task);
  }

  today.sort((a, b) => {
    const aTime = a.routine?.targetTime ?? "23:59";
    const bTime = b.routine?.targetTime ?? "23:59";
  ...
  const mapTask = (task: any) => {
    const rt = task.routine;
    const mapped: Record<string, unknown> = {
      id: task.id,
      title: task.title,
      status: task.status,
      scheduledDate: task.scheduledDate?.toISOString() ?? null,
      estimatedMins: task.estimatedMins ?? rt?.estimatedMins ?? null,
      energyLevel: task.energyLevel,
      context: task.context,
      project: task.project,
      isOverdueWithinDay: rt?.dueByTime ? nowHHMM > rt.dueByTime : false,
    };
    if (rt) {
      mapped.routine = {
        id: rt.id,
        scheduleLabel: scheduleLabel(rt.cronExpression),
        color: rt.color,
        skipStreak: rt.skipStreak,
        targetTime: rt.targetTime,
        dueByTime: rt.dueByTime,
      };
      if (rt.progressionBaseValue != null && rt.progressionIncrement != null && rt.progressionUnit && rt.progressionFrequency && rt.progressionStartDate) {
        const currentValue = getProgressionValue({
          baseValue: rt.progressionBaseValue,
          increment: rt.progressionIncrement,
          unit: rt.progressionUnit,
          frequency: rt.progressionFrequency,
          startDate: rt.progressionStartDate,
        });
        (mapped.routine as Record<string, unknown>).progression = {
          currentValue,
          unit: rt.progressionUnit,
          label: formatProgression(currentValue, rt.progressionIncrement, rt.progressionUnit, rt.progressionFrequency),
        };
      }
    }
    return mapped;
    ...
    try {
      ...
      switch (name) {
        ...
        case "tandem_project_create_from_template":
          ...
          break;
        // Decision tools
        case "tandem_decision_create":
          result = await handleDecisionCreate(
            args as Parameters<typeof handleDecisionCreate>[0]
          );
          break;
        case "tandem_decision_respond":
          result = await handleDecisionRespond(
            args as Parameters<typeof handleDecisionRespond>[0]
          );
          break;
        case "tandem_decision_list_pending":
          result = await handleDecisionListPending();
          break;
        case "tandem_decision_resolve":
          result = await handleDecisionResolve(
            args as Parameters<typeof handleDecisionResolve>[0]
          );
          break;
        case "tandem_decision_vote_option":
          result = await handleDecisionVoteOption(
            args as Parameters<typeof handleDecisionVoteOption>[0]
          );
          break;
        case "tandem_decision_list":
          result = await handleDecisionList(
            args as Parameters<typeof handleDecisionList>[0]
          );
          break;
        // Thread tools
        case "tandem_thread_create":
          result = await handleThreadCreate(
            args as Parameters<typeof handleThreadCreate>[0]
          );
          break;
        case "tandem_thread_list":
          result = await handleThreadList(
            args as Parameters<typeof handleThreadList>[0]
          );
          break;
        case "tandem_thread_resolve":
          result = await handleThreadResolve(
            args as Parameters<typeof handleThreadResolve>[0]
          );
          break;
        case "tandem_thread_reply":
          result = await handleThreadReply(
            args as Parameters<typeof handleThreadReply>[0]
          );
          break;
        case "tandem_routine_list":
        case "tandem_recurring_template_list": // backward compat alias
          result = await handleRoutineList(
            args as Parameters<typeof handleRoutineList>[0]
          );
          break;
        case "tandem_routine_create":
        case "tandem_recurring_template_create": // backward compat alias
          result = await handleRoutineCreate(
            args as Parameters<typeof handleRoutineCreate>[0]
          );
          break;
        case "tandem_routine_update":
        case "tandem_recurring_template_update": // backward compat alias
          result = await handleRoutineUpdate(
            args as Parameters<typeof handleRoutineUpdate>[0]
          );
          break;
        case "tandem_routine_delete":
        case "tandem_recurring_template_delete": // backward compat alias
          result = await handleRoutineDelete(
            args as Parameters<typeof handleRoutineDelete>[0]
          );
          break;
        case "tandem_card_file_list":
          result = await handleCardFileList();
          break;
        default:
          return {
            content: [
