...
function flattenResults(results: SearchResults): FlatItem[] {
  ...
  for (const th of (results.threads ?? [])) items.push({ type: "thread", data: th });
  return items;
}
...
function ThreadItem({ data }: { data: ThreadResult }) {
  return (
    <div className="flex items-center gap-3 min-w-0">
...
export function GlobalSearch() {
  ...
  function renderResults() {
    ...
    renderSection("Threads", results.threads ?? [], (item, idx) => (
      <div
        key={item.id}
        ref={(el) => {
          if (el) itemRefs.current.set(idx, el);
        ...
        onClick={() => navigateTo({ type: "thread", data: item })}
        onMouseEnter={() => setActiveIndex(idx)}
      >
        <div className="flex-1 min-w-0">
