...
interface RSVPFormByEventProps {
  eventId: string;
  userEmail: string;
}

export function RSVPFormByEvent({ eventId, userEmail }: RSVPFormByEventProps) {
  ...
  const fetchEvent = useCallback(async () => {
    try {
      ...
      if (!res.ok) {
        ...
        setError(data.error || "Unable to load this event.");
        return;
      }
      const data = await res.json();
      setEvent(data.event);
      setInvitation(data.invitation);
      // Pre-fill if they already responded
      if (data.existingResponse) {
        setAttendance(data.existingResponse.attendance);
        setFieldValues(data.existingResponse.fieldValues || {});
      }
    } catch {
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          {event.fields.map((field) => {
            if (field.type === "ATTENDANCE") return null;
