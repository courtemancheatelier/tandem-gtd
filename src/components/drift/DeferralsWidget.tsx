...
export function DeferralsWidget({ current, prior }: Props) {
    ...
    return (
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">Skipped / Deferred</CardTitle>
          <CardDescription>No skipped or deferred commitments in this period</CardDescription>
        </CardHeader>
      </Card>
    );
  }
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="text-base">Skipped / Deferred</CardTitle>
        <CardDescription>
          {currentTotal} skipped or deferred
          {priorTotal > 0 && (
            <span className={trend <= 0 ? "text-green-600 ml-2" : "text-amber-500 ml-2"}>
        ...
        <div className="h-[200px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data}>
              ...
              />
              <Legend />
              <Bar dataKey="current" fill="hsl(var(--chart-4, 43 96% 56%))" name="Current" radius={[2, 2, 0, 0]} />
