    ...
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2 mt-2 flex-wrap">
            ...
            {editingStartDate ? (
              <div className="flex items-center gap-1">
                <input
                  ...
                  onBlur={(e) => {
                    ...
                    if (val) {
                      onUpdateProject({ startDate: new Date(val + "T00:00:00Z").toISOString() });
                    }
                  }}
                  onKeyDown={(e) => {
              ...
              <div className="flex items-center gap-0.5">
                ...
                <button
                  onClick={() => onUpdateProject({ startDate: null })}
            ...
            {editingEndDate ? (
              <div className="flex items-center gap-1">
                <input
                  ...
                  onBlur={(e) => {
                    ...
                    const val = e.target.value;
                    if (val) {
                      onUpdateProject({ endDate: new Date(val + "T00:00:00Z").toISOString() });
                    }
                  }}
                  onKeyDown={(e) => {
                    if (e.key === "Enter") (e.target as HTMLInputElement).blur();
              ...
              <div className="flex items-center gap-0.5">
                ...
                <button
                  onClick={() => onUpdateProject({ endDate: null })}
                  className="text-muted-foreground hover:text-foreground p-0.5 rounded"
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          />
          {(onSaveAsTemplate || onMoveProject || (parentProject && onMakeStandalone) || hasChildProjects) && (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
              ...
              <DropdownMenuContent align="end">
                ...
                {hasChildProjects && (
                  <DropdownMenuItem onClick={() => onUpdateProject({ outlineReady: !outlineReady })}>
                    <FileText className="h-4 w-4 mr-2" />
                    {outlineReady ? "Disable outline as default" : "Use outline as default"}
                  </DropdownMenuItem>
                )}
              </DropdownMenuContent>
            </DropdownMenu>
      ...
      {/* External link */}
      <ExternalLinkField
        url={project.externalLinkUrl ?? null}
        label={project.externalLinkLabel ?? null}
        onSave={async (url, label) => {
          await onUpdateProject({ externalLinkUrl: url, externalLinkLabel: label });
        }}
      />
