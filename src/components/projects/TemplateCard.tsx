...
export function TemplateCard({ template, onClick, isHidden }: TemplateCardProps) {
  ...
  const teamName = template.team?.name;

  return (
    <Card
      className={`cursor-pointer hover:border-primary/50 transition-colors p-4 ${isHidden ? "opacity-50" : ""}`}
      onClick={onClick}
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {teamName && (
            <Badge variant="secondary" className="text-xs gap-1 py-0 mt-0.5">
