...
import { convertHtmlLinksToMarkdown } from "@/lib/paste-links"
...
const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(
  ({ className, onChange, onKeyDown, teamId, ...props }, forwardedRef) => {
    ...
    const handleChange = React.useCallback(
      ...
      [onChange, acHandleChange, autoResize]
    )
    ...
    const handlePaste = React.useCallback(
      (e: React.ClipboardEvent<HTMLTextAreaElement>) => {
        ...
        const converted = convertHtmlLinksToMarkdown(html);
    ...
    return (
      <div className="relative">
        <textarea
          className={cn(
            "flex min-h-[60px] w-full rounded-md border border-input bg-background px-3 py-2 text-base md:text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 overflow-hidden resize-none",
            className
          )}
          ref={mergedRef}
