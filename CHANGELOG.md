...
## v1.9.1 — Security backport

Security hygiene fixes ported from the v2.0 review. No feature changes.

- **Team template role check (MEDIUM)** — saving a project as a team template now requires ADMIN role on the target team; previously any team member could create a team template
- **Timing-safe CRON_SECRET comparison (MEDIUM)** — trial, recurring-missed, and notifications cron routes now compare the secret with `crypto.timingSafeEqual` via a new `verifyCronSecret()` helper, closing a narrow timing side-channel
- **Path traversal guard in template loader (LOW)** — `loadTemplateMeta` now resolves and normalizes the target path and rejects anything outside `docs/templates/`
- **Task-template array bound (LOW)** — `createProjectTemplateSchema.taskTemplates` now enforces `.max(100)` to prevent unbounded payloads
- **`requireAdmin` Bearer token support (LOW)** — admin-only routes now accept both session cookies and Bearer tokens via `requireAuth`
- **Encryption-key fallback warning (LOW)** — logs a one-time warning when `TANDEM_ENCRYPTION_KEY` is unset and encryption falls back to `NEXTAUTH_SECRET`, so operators can notice and set a dedicated key in production
