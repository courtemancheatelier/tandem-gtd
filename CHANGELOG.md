...
### v1.9 — Polish, Integrations & Communication

#### Mobile & Polish
- Mobile polish — swipe-to-complete, pull-to-refresh, haptic feedback, offline indicator, safe area insets

#### Integrations
- Calendar + Google Sync — native hard-landscape events, time blocking UI, bidirectional Google Calendar sync
- Email-to-inbox capture — inbound email processing via Cloudflare Workers, per-user inbox address

#### Team & Communication
- Team Sync — work-anchored communication in 3 layers: enriched events, work-anchored threads, decision requests. Pull-not-push model — no chat, everything resolves through GTD flows.
...
#### Admin Settings
- Admin settings reorganization — group related sections with collapsible panels, improve section ordering

#### Time & Focus
- Focus timer — gentle, opt-in focus timer with floating pill indicator, pause/resume, auto-pause on task switch
- Task duration tracking — close the estimation feedback loop by capturing actual time on task completion
- Time audit challenge — one-week awareness exercise logging activities every 15 minutes

### v2.0+ — Growth

- Decision Proposals — full PR-style async decision workflow with structured voting, research phase, wiki auto-update
...
- Volunteer & nonprofit orgs — contributor experience level, volunteer onboarding, board governance reports
