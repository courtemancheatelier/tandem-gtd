...
Don't want to self-host? **Tandem Managed Hosting** handles provisioning, updates, backups, and infrastructure so you can focus on your work. The commercial pipeline — trial tracking, checkout, server provisioning, and customer portal — is developed separately in [tandem-manage](https://github.com/courtemancheatelier/tandem-manage) (private). Commercial specs (launch strategy, pricing, evaluation mode, waitlists, etc.) live in that repo; pointer files in `docs/` link to them.
